...
for epoch in range(num_epochs):
    for _ in range(n_bs):        # for each batch
        ...
        d = dis(theta_half, x) - dis(theta_half, gen(phi_half.float(), Z))
        d.backward()
        theta.data = theta.data + alpha * theta_half.grad
        theta.requires_grad_()
        # y^{t+1} = y^t - alpha * nabla_y f(x^{t+1/2}, y^{t+1/2})
        phi.data = phi.data - alpha * phi_half.grad
        phi.requires_grad_()
...
np.save("egJ_gamma_" + str(gamma) + "_alpha_" + str(alpha), Phi)
