...
for epoch in range(num_epochs):
    for _ in range(n_bs):        # for each batch
        ...
        d = dis(theta, x) - dis(theta, gen(phi.float().detach(), Z))
        d.backward()
        # compute nabla_x f(x^{t-1}, y^{t-1})
        d = dis(oldtheta, x) - dis(oldtheta, gen(oldphi.float().detach(), Z))
        d.backward()
        theta_tmp = theta.float() + alpha * theta.grad.float() - beta1 * oldtheta.grad.float()

        # rename oldtheta
        oldtheta = theta.detach()
        oldtheta.requires_grad_()
        # rename theta
        theta = theta_tmp.detach()
        theta.requires_grad_()

        # compute nabla_y f(x^{t+1}, y^t), renew phi
        d = - dis(theta.detach().float(), gen(phi.float(), Z))
        d.backward()
        # compute nabla_y f(x^t, y^{t-1})
        d = -dis(oldtheta.detach(), gen(oldphi.float(), Z))
        d.backward()
        phi_temp = phi.float() - alpha * phi.grad.float() + beta2 * oldphi.grad.float()
...
np.save("ogd_alpha_" + str(alpha) + "_beta1_" + str(beta1) + "_beta2_" + str(beta2), Phi)
