...
def train_dis(dis, real_data, fake_data, oldgrad, ss=0.01):
    ...
    for n, params in enumerate(dis.parameters()):
        params.data = params.data - 2 * ss * params.grad + 1 * ss * oldgrad[n]

    return error_real + error_fake, prediction_real, prediction_fake


def train_gen(gen, fake_data, oldgrad, ss=0.01):
    ...
    for n, params in enumerate(gen.parameters()):
        params.data = params.data - 2 * ss * params.grad + ss * oldgrad[n]
...
for epoch in range(num_epochs):
    for n_batch, zb in enumerate(data_loader):
        ...
        d_error, d_pred_real, d_pred_fake = train_dis(dis, real_data, fake_data, oldgrad_d)
        ...
        g_error = train_gen(gen, fake_data, oldgrad_g)
	    # record the last step grad
        for n, pa in enumerate(gen.parameters()):
        ...
        if (n_batch) % 100 == 0:
            ...
            plt.savefig('./gmm_ogd/epoch_' + str(epoch) + '_batch_' + str(n_batch) + '.pdf')   # save image
            # Display status Logs
            logger.display_status(epoch, num_epochs, n_batch, num_batches, d_error, g_error, d_pred_real, \
                    d_pred_fake)

np.save("gmm_ogd_lr_" + str(lr), Phi)
